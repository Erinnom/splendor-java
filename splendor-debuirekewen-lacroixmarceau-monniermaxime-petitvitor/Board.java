...
public class Board implements Displayable {
    ...
    public void updateCard(DevCard card) {
        ...
        if (visibleCards[tier_card - 1][i] == card) {
            if (stackCards.get(tier_card - 1).size() != 0) {
                visibleCards[tier_card - 1][i] = stackCards
                    .get(tier_card - 1)
                    .pop();
            } else {
    ...
    private String[] boardToStringArray() {
        ...
        for (int i = 1; i < stackCards.size() + 1; i++) {
            deckDisplay = Display.concatStringArray(
                deckDisplay,
                deckToStringArray(i),
                true
            );
            ...
            for (int j = 0; j < 4; j++) { //-- parcourir les 4 cartes faces visibles pour un niveau donné (j)
                tierCardsDisplay = Display.concatStringArray(
                    ...
                    visibleCards[i][j] != null
                        ? visibleCards[i][j].toStringArray()
                        : DevCard.noCardStringArray(),
                    false
                );
            }
            cardDisplay = Display.concatStringArray(
                cardDisplay,
                Display.emptyStringArray(1, 40),
                true
            );
            cardDisplay = Display.concatStringArray(
                cardDisplay,
        ...
        res = Display.concatStringArray(
            res,
            Display.emptyStringArray(1, 52),
            true
        );
        res = Display.concatStringArray(res, resourcesToStringArray(), true);
        res = Display.concatStringArray(
            res,
            Display.emptyStringArray(35, 1, " \u250A"),
            false
        );
        res = Display.concatStringArray(
            res,
            Display.emptyStringArray(1, 54, "\u2509"),
            true
        );
        return res;
    }
