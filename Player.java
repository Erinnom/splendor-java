...
public abstract class Player implements Displayable {
    ...
    public String[] toStringArray(){
        ...
        for(Resource res: Resource.values()){ //-- parcourir l'ensemble des resources (res) en utilisant l'énumération Resource
            ...
            System.out.println(3+(Resource.values().length-1-res.ordinal()));
    ...
    public boolean canBuyCard(DevCard dev){
        if(getNbResource(Resource.DIAMOND) < dev.getCost().getNbResource(Resource.DIAMOND)){
            return false;
        }
        if(getNbResource(Resource.SAPPHIRE) < dev.getCost().getNbResource(Resource.SAPPHIRE)){
            return false;
        }
        if(getNbResource(Resource.EMERALD) < dev.getCost().getNbResource(Resource.EMERALD)){
            return false;
        }
        if(getNbResource(Resource.ONYX) < dev.getCost().getNbResource(Resource.ONYX)){
            return false;
        }
        if(getNbResource(Resource.RUBY) < dev.getCost().getNbResource(Resource.RUBY)){
            return false;
        }
        return true;
    }
